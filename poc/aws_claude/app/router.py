...
Runs on **Amazon Nova Micro** on Bedrock Singapore (`apac.amazon.nova-micro-v1:0`).
~30× cheaper than Claude Haiku 4.5 on input and fine for a deterministic
classifier that always emits the same JSON shape.
"""
...
BEDROCK_REGION = os.environ.get("BEDROCK_CLAUDE_REGION", "ap-southeast-1")
...
- `secondary` is a list (possibly empty) of departments that must be consulted as a side-channel — e.g. include "infectious-disease" for any antibiotic question.
- Never pick "emergency" here — emergency cases come in with a hard toggle flag and skip this router entirely.
